...
export default function BillPage() {
  ...
  const { list = [], actionLoading = false } = useSelector((state) => state.bills) || {};
  ...
  const filteredList = useMemo(() => list.filter((b) => b.month === selectedMonth), [list, selectedMonth]);

  // Summary
  const summary = useMemo(() => {
    const names = ["Dukan vara", "WiFi", "Biddut (Electricity)"];
    const paidThisMonth = filteredList.reduce((s, b) => s + Number(b.amount || 0), 0);
    const unpaidCount = names.reduce((cnt, name) => (filteredList.find((b) => b.name === name) ? cnt : cnt + 1), 0);

    const lastMonth = dayjs(selectedMonth + "-01").subtract(1, "month").format("YYYY-MM");
    const lastList = list.filter((b) => b.month === lastMonth);
    const lastPaid = lastList.reduce((s, b) => s + Number(b.amount || 0), 0);
    const lastUnpaidCount = names.reduce((cnt, name) => (lastList.find((b) => b.name === name) ? cnt : cnt + 1), 0);

    return { paidThisMonth, unpaidCount, lastPaid, lastUnpaidCount, totalBills: list.length };
  ...
  const billsWithStatus = useMemo(() => {
    ...
    return names.map((name) => {
      const current = list.find((b) => b.name === name && b.month === selectedMonth);
      const last = list.find((b) => b.name === name && b.month === lastMonth);
      return {
        name,
    ...
    <div className="min-h-screen p-6 bg-[#0D0D10] text-gray-200 max-w-6xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-1">Manage monthly bills and last-month dues.</p>
        </div>

        <div className="flex items-center gap-3">
          <label className="text-sm text-gray-400">Month:</label>
          <select value={selectedMonth} onChange={(e) => setSelectedMonth(e.target.value)} className="bg-[#0f0f14] border border-gray-700 p-2 rounded">
            {monthOptions.map((m) => (
              <option key={m} value={m}>{dayjs(m + "-01").format("MMMM YYYY")}</option>
            ))}
          </select>

          <button onClick={() => { setForm({ name: "", amount: "", isLastMonthUpdate: false }); setEditId(null); setOpenModal(true); }} className="bg-blue-600 px-4 py-2 rounded-lg shadow hover:bg-blue-700">
            + Add Bill
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4 mb-6">
        ...
        <div className="bg-[#121217] p-4 rounded-lg shadow border border-gray-800">
          ...
          <div className="text-2xl font-bold mt-2">{summary.lastPaid} Tk</div>
        </div>
        <div className="bg-[#121217] p-4 rounded-lg shadow border border-gray-800">
            ...
            {billsWithStatus.map((bill, idx) => (
              <tr key={idx} className="hover:bg-[#1b1b25] transition-all border-b border-gray-800">
                ...
                <td className="p-3 text-center flex justify-center gap-2">
                  {bill.current._id && (
                    <>
                      <button onClick={() => startEdit({ _id: bill.current._id, name: bill.name, amount: bill.current.amount })} className="px-3 py-1 bg-blue-600 rounded-lg hover:bg-blue-700">Edit</button>
                      <button onClick={() => handleDelete(bill.current._id)} className="px-3 py-1 bg-red-600 rounded-lg hover:bg-red-700">Delete</button>
                    </>
                  )}
                </td>
              </tr>
      ...
      {openModal && (
        <div className="fixed inset-0 bg-black/60 flex justify-center items-center z-50">
          <div className="bg-[#1a1a22] p-6 rounded-xl w-96 shadow-2xl border border-gray-700">
            <h3 className="text-xl font-bold mb-4">{editId ? "✏ Update Bill" : form.isLastMonthUpdate ? "🕒 Add Last Month Due" : "➕ Add Bill"}</h3>
            <select value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="bg-[#0f0f14] border border-gray-700 p-3 rounded-lg w-full mb-3">
              <option value="">Select Bill</option>
              <option value="Dukan vara">Dukan vara</option>
              <option value="WiFi">WiFi</option>
              <option value="Biddut (Electricity)">Biddut (Electricity)</option>
            </select>
            <input type="number" placeholder="Amount" value={form.amount} onChange={(e) => setForm({ ...form, amount: e.target.value })} className="bg-[#0f0f14] border border-gray-700 p-3 rounded-lg w-full mb-4" />

            <div className="flex justify-end gap-3">
              <button onClick={() => { setOpenModal(false); setEditId(null); setForm({ name: "", amount: "", isLastMonthUpdate: false }); }} className="px-4 py-2 bg-gray-700 rounded-lg hover:bg-gray-600">Cancel</button>
              <button onClick={submitBill} className="px-4 py-2 bg-blue-600 rounded-lg hover:bg-blue-700">{actionLoading ? "Saving…" : editId ? "Update" : "Add"}</button>
            </div>
          </div>
        </div>
      )}
