...
export default function SellPage() {
  ...
  const products = useSelector(
    (state) => (state?.products?.items || []).filter((p) => p.stock > 0)
  );

  const [customer, setCustomer] = useState({ name: "", phone: "" });

  const [itemMemo, setItemsMemo] = useState([
    { name: "", qty: 1, sellPrice: 0, price: 0, total: 0 },
  ]);
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await dispatch(addSale(saleData)).unwrap();
      alert("✅ Sale saved successfully!");
      router.push(`/sale/${res._id}`); // redirect to sale/id
